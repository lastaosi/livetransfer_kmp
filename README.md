...
> Android는 전체 기능 구현 완료. iOS는 Koin DI 초기화 및 날씨·환율 ViewModel 구현 진행 중입니다.
...
| 네트워크 | Ktor (Android: OkHttp, iOS: Darwin), kotlinx.serialization |
...
│       │   ├── remote/   # KTorClient (expect/actual, Ktor HttpClient 싱글톤)
│       │   ├── repository/
│       │   │   ├── ExchangeRepositoryImpl.kt   # Frankfurter API
│       │   │   └── WeatherRepositoryImpl.kt    # OpenWeatherMap 호출 (공유)
│       │   └── source/local/
│       │       └── CityDataStore.kt  # expect class (플랫폼별 구현)
│       └── domain/
...
│       ├── data/
│       │   ├── remote/
│       │   │   └── KTorClient.android.kt  # actual (OkHttp 엔진)
│       │   ├── repository/
│       │   │   ├── TranslationRepositoryImpl.kt  # Gemini 호출
│       │   │   └── SettingsRepository.kt          # 언어/속도 설정 DataStore
│       │   └── source/local/
│       │       └── CityDataStore.kt  # actual class (DataStore Preferences)
│       ├── di/
...
└── iosMain/              # iOS 구현
    └── kotlin/com/jh/livetransfer/
        ├── MainViewController.kt       # Koin 초기화 후 진입점
        ├── data/
        │   ├── remote/
        │   │   └── KTorClient.ios.kt   # actual (Darwin 엔진)
        │   └── source/local/
        │       └── CityDataStore.kt    # actual class (NSUserDefaults)
        ├── di/
        │   └── KoinInitializer.kt      # iOS용 Koin 모듈 초기화
        └── ui/screen/
            ├── weather/
            │   └── WeatherViewModelIos.kt    # iOS ViewModel (진행 중)
            └── exchangerate/
                └── ExchangeViewModelIos.kt   # iOS 환율 ViewModel
```
...
| iOS 구현 | Koin DI·ViewModel 골격 완료. UI 화면 및 번역·TTS 기능 미완성 |
